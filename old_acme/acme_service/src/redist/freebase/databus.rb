...
#    
# $Id: databus.rb,v 1.1 2004-07-26 17:09:24 wwright Exp $
#
  ...
  class DataBus
    ...
    class Slot
      ...
      def has_child?(name=nil)
        ...
        result = @slots[name] unless result
      ...
      def subscribe(subscriber=nil, &block)
        if subscriber
          unless subscriber.respond_to?("databus_notify")
            raise "Subscribers must implment the databus_notify method" 
          end
          @subscribers[subscriber] = subscriber
          return subscriber
        else
          count = @databus.idCount
          @subscribers[count] = Proc.new(&block)
          return count
        end
      end
