...
module REXML
	...
	class Element < Parent
		...
		# indent:: 
		#   if supplied, must be an Integer, which is then used
		...
		# Parses a stream containing a single top-level element and possibly
		# multiple children.
		# source:: A REXML::Source to read from
		# listener:: 
		#   A REXML::StreamListener (or something that implements the same method 
		#   set) that will be notified when events occur.
		...
		def Element.parse_stream source, listener
			begin
				...
				unless closed
					while true
						...
						when Instruction::START_RE
							...
							raise ParseException.new("malformed XML: bad content", source ) if md[1][0] == ?<
				...
				$!.source = source
				$!.element = self if self.kind_of? Element
				raise
			rescue Exception
				raise
				old_ex = $!
		...
		# parse_stream.
		#
		# Returns:: an array with [tag_name_S, closed_B, attributes_A]
		...
		def Element.base_parser source
			...
			return [md[1], md[4], attrs]
		end

		# Does the actual work of parsing the element.
		def parse source
			# Get the next tag
			self.name, closed, attrs = Element.base_parser(source)
			attrs.each do |set|
				a = Attribute.new( set[0], set[2] )
				a.normalized = true
				@attributes[set[0]] = a
			end
			...
			unless closed
				...
				raise ParseException.new( "Missing end tag for #{expanded_name}", 
					source, self) unless @expanded_name == md[1]
			end
		rescue ParseException
			$!.source = source
			$!.element = self
			...
			while true
				...
				if source.buffer[0] == ?<
					if source.buffer[1] == ?/
						...
						raise ParseException.new("Malformed node",source,self) unless md
						case md[1]
						when CData::START_RE
						...
						when Comment::START_RE
							...
							raise ParseException.new("malformed XML: bad content", source, self )
						end
					elsif source.buffer[1] == ??
					...
					t = Text.new(source,whitespace(),raw())
					add(t) unless ignore_whitespace_nodes and t.string.strip.empty?
				end
			end
		end

		# A private helper method
		def write_children( writer, indent, transitive )
			cr = (indent < 0) ? '' : "\n"
			#if size == 1 and @children[0].kind_of?(Text)
			#	self[0].write( writer, -1 )
			if indent == -1
				each { |child| child.write( writer, indent, transitive ) }
			else
				...
				each { |child|
					...
					child.write( writer, next_indent, transitive )
					last_child = child
				}
	...
	class Attributes < Hash
		...
		def get_attribute( name )
			...
			if attr.nil?
				...
				if @element.document and @element.document.doc_type
					...
					attr_val = @element.document.doc_type.attribute_of(expn, name)
		...
		def []=( name, value )
			...
			if value.prefix == 'xmlns' or value.name == 'xmlns'
				raise ParseException.new("Duplicate namespace declared: #{value}") if @element.namespaces.include?( value.value )
			end
